...
(function() {
    ...
    function extractGraduationYear(text) {
        ...
        console.log(`找到${educationSections.length}个可能的教育背景区域`);
        
        // 排除可能属于"其他名校毕业的牛人"板块的教育区域
        const validEducationSections = educationSections.filter(section => {
            // 检查此教育区域是否在推荐区域内
            const isInRecommendation = section.closest('.recommend-container') || 
                                       section.parentElement?.textContent.includes('其他名校毕业的牛人');
            
            if (isInRecommendation) {
                console.log('排除推荐区域中的教育信息');
                return false;
            }
            return true;
        });
        
        console.log(`筛选后剩余${validEducationSections.length}个有效教育背景区域`);
        
        let years = [];
        
        // 从已识别的教育区域中提取年份
        for (const section of validEducationSections) {
            console.log(`处理教育区域: ${section.tagName}#${section.id}.${section.className}`);
            
            // 寻找时间段标记（通常包含毕业年份）
            const timeElements = section.querySelectorAll('.time-range, .date, .year, .period');
            for (const el of timeElements) {
                const text = el.textContent.trim();
                const yearMatches = text.match(/20\d\d|19\d\d/g);
                if (yearMatches) {
                    console.log(`在教育时间元素中找到年份: ${yearMatches.join(', ')}`);
                    years = years.concat(yearMatches.map(y => parseInt(y)));
                }
            }
            ...
            for (const pattern of yearRangePatterns) {
                ...
                for (const match of matches) {
                    ...
                    if (endYear && endYear >= 1980 && endYear <= 2030) {
                        console.log(`在教育区域文本中找到年份范围: ${match[0]}, 结束年份: ${endYear}`);
                        years.push(endYear);
                    }
                }
            }
            ...
            // 检查特定的学历条目
            const educationItems = section.querySelectorAll('li, .item, .edu-item, .experience-item');
            for (const item of educationItems) {
                const itemText = item.textContent;
                for (const pattern of yearRangePatterns) {
                    const matches = Array.from(itemText.matchAll(pattern));
                    for (const match of matches) {
                        let endYear;
                        if (match[0].includes('至今') || match[0].includes('现在') || match[0].includes('进行中')) {
                            endYear = new Date().getFullYear();
                        ...
                        if (endYear && endYear >= 1980 && endYear <= 2030) {
                            console.log(`在教育条目中找到年份范围: ${match[0]}, 结束年份: ${endYear}`);
                            years.push(endYear);
                        }
                    }
                }
            ...
            // 如果未找到年份范围，查找单独的年份（如毕业时间:"2022"）
            if (years.length === 0) {
                const yearPattern = /(?:毕业|结束|完成)[^，,。]*?([12]\d{3})|([12]\d{3})(?:年|\s)?(?:毕业|结束|完成)/g;
                const matches = Array.from(sectionText.matchAll(yearPattern));
                for (const match of matches) {
                    const year = parseInt(match[1] || match[2]);
                    if (year && year >= 1980 && year <= 2030) {
                        console.log(`在教育区域中找到毕业年份标记: ${match[0]}, 年份: ${year}`);
                        years.push(year);
                    }
                }
            }
            
            // 最后寻找单独的年份数字，但仅在确定的教育区域内
            if (years.length === 0) {
                const singleYearPattern = /[学校院][^，,。]*?([12]\d{3})/g;
                const yearMatches = Array.from(sectionText.matchAll(singleYearPattern));
                for (const match of yearMatches) {
                    const year = parseInt(match[1]);
                    if (year >= 1980 && year <= 2030) {
                        console.log(`在学校名称附近找到年份: ${match[0]}, 年份: ${year}`);
                        years.push(year);
                    }
                }
            }
        ...
        // 检查是否是应届生
        const freshGradPatterns = [
            /(\d{2,4})[\s年]应届/,            // 2025年应届、25年应届
            /应届[\s生].*?(\d{2,4})[\s年]/,   // 应届生2025年、应届 25年 
            /应届[\s生].*?(\d{4})/            // 应届生2025
        ];
        ...
        for (const section of validEducationSections) {
            const sectionText = section.textContent;
            for (const pattern of freshGradPatterns) {
    ...
    function extractYearFromEducationSection(educationSection) {
        ...
        // 3. 检查教育条目列表
        const educationItems = educationSection.querySelectorAll('li, .item, .edu-item, .experience-item');
        for (const item of educationItems) {
